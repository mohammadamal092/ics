...
function basePath(){
  ...
  // Use the first URL segment as the repository base path.
  if(location.hostname.endsWith('.github.io')){
    const parts=location.pathname.split('/').filter(Boolean);
    return parts.length ? '/'+parts[0]+'/' : '/';
  }
  // Local development (VS Code Live Server, localhost, etc.)
  return location.pathname.includes('/ICS_GitHub/') ? '/ICS_GitHub/' : '/';
}
